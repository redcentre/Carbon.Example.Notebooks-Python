...
def carboncrosstab(topseries : pd.Series, sideseries : pd.Series, colpercents : bool=False,natsort : bool =False,dps:int=2): #,basis : pd.Series = pd.Series([1.0 for x in range(len(df.index))])):
    ...
    try:
        response = requests.post('https://rcsapps.azurewebsites.net/carbon/python/crosstab/alphacodes', json=postdata,headers=headers)
        if colpercents:
        ...
        return requests.post('https://rcsapps.azurewebsites.net/carbon/python/crosstab/alphacodes', json=postdata,headers=headers)
